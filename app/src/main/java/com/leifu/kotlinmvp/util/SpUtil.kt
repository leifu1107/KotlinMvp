...
object SpUtil {
    ...
    val appSpRecord: SharedPreferences
        get() = MyApp.context.getSharedPreferences("sp_searh", Context.MODE_PRIVATE)

    fun putSearchString(key: String, value: String) {
        appSpRecord.edit().putString(key, value).apply()
    }

    fun getSearchList(): ArrayList<String> {
        val map = appSpRecord.all
        val list = arrayListOf<String>()
        map.entries.forEach { list.add(it.value as String) }
        return list
    }
}
